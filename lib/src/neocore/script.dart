...
class ScriptBuilder {
  ...
  pushBool(bool b) {
    pushNum(x: b ? OpCode.pusht : OpCode.pushf);
  }
  ...
  pushVarInt(int x) {
    if (x < 0xfd) {
      ...
      pushNum(x: 0xfd);
      pushNum(x: x, len: 2, bigEndian: false);
    } else if (x < 0xffffffff) {
      pushNum(x: 0xfe);
      pushNum(x: x, len: 4, bigEndian: false);
    } else {
      pushNum(x: 0xff);
      pushNum(x: x, len: 8, bigEndian: false);
    }
  }
  ...
  pushHex(Uint8List hex) {
    ...
    if (len < OpCode.pushbytes75) {
      ...
      pushNum(x: OpCode.pushdata1);
      pushNum(x: len);
    } else if (len < 0x10000) {
      pushNum(x: OpCode.pushdata2);
      pushNum(x: len, len: 2, bigEndian: false);
    } else {
      pushNum(x: OpCode.pushdata4);
  ...
  pushBigInt(BigInt x) async {
    if (x == BigInt.from(-1)) {
      pushNum(x: OpCode.pushm1);
    } else if (x == BigInt.from(0)) {
      pushNum(x: OpCode.push0);
    } else if (x > BigInt.from(0) && x < BigInt.from(16)) {
      pushNum(x: OpCode.push1 - 1 + x.toInt());
    } else {
  ...
  pushStr(String str) {
    pushHex(Uint8List.fromList(Utf8Encoder().convert(str)));
  }
}
